...
memento assumes you have an user-model. Every session is owned by an user.
...
This will take an snapshot only when an instance is updated or destroyed.
...
This will ignore changes on the calculated_birthday and the firends_count-attributes. When memento saves an whole instance of your model before it is destroyed, this attributes will not be stored for later recovery. Only ignore attributes you can re-calculate from other data!
...
Copyright (c) 2009 Yolk Sebastian Munz & Julia Soergel GbR. See LICENSE for details.
